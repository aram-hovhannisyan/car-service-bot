...
def send_welcome(message):
    ...
    if user:
        ...
        service_name = "Пример автосервиса"
        work_schedule = "Понедельник-пятница, с 9:00 до 18:00"
        address = "ул. Примерная, 123, Город, Страна"
        description = "Добро пожаловать в Примерный автосервис! Мы предоставляем широкий спектр автомобильных услуг."
...
def get_year_of_issue(message, order_details):
    ...
    bot.send_message(message.chat.id, "Кто будет покупать расходные материалы, купите вы или дайте купить нам?")
...
def get_model(message, order_details):
    ...
    bot.send_message(message.chat.id, "Пожалуйста, выберите планируемую дату проведения службы:", reply_markup=generate_inline_calendar())
...
def process_feedback(message):
    ...
    if 1 <= rating <=5:
        ...
        bot.send_message(message.chat.id, "Спасибо за ваш отзыв!", reply_markup=inline_keyboard)
    else:
