...
def create_pagination_buttons(page, menu_type):
    ...
    prev_button = types.InlineKeyboardButton(f"◀️ Предыдущий {max(1, page-1)}", callback_data=f"{menu_type}_{max(1, page-1)}")
    current = types.InlineKeyboardButton(f"{page}/{page_count}", callback_data=f"x")
    next_button = types.InlineKeyboardButton(f"Следующий {min(page_count, page + 1)}▶️", callback_data=f"{menu_type}_{min(page_count, page + 1)}")
