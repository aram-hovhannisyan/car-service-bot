...
def admin_commands(message):
    ...
    bot.send_message(message.chat.id, "Admin Interface:", reply_markup=admin_menu)
...
def admin_view_archived_orders(message, page):
    orders_message = "Вот все услуги, которые были заархивированыы:\n\n"
...
def set_order_status(message, new_status, order_id):
    ...
    if new_status == "В архиве":
        status_message = "Ваш заказ заархивирован"
        ...
        status_message = "Ваш заказ не был выполнен: "
        ...
        status_message = "Ваш заказ был отменен"
...
def admin_faq_button(message ,page):
    ...
    if not faqs:
        bot.send_message(message.chat.id, "No FAQs available.")
        return
    inline_keyboard = types.InlineKeyboardMarkup(row_width=2)
